<div id="top-panel">
   <img src="/images/logo.svg" style="width: 97px;height: 33px;" alt="Logo" class="float-start" id="logo">
   ...
   <div class="btn-group me-3 float-end" role="group">
      <button class="btn btn-primary btn-icon" title="Export (Ctrl + E)" id="save-btn" data-vvveb-action="saveAjax" data-vvveb-url="{{route('public.editor.store', $page->id)}}" data-v-vvveb-shortcut="ctrl+e">
      <i class="la la-save"></i> <span data-v-gettext>Save page</span>
      </button>
	  @if($type == "post")
		   <a  href="{{route('blog.post.list')}}" style="margin-left:3px;"class="btn btn-primary btn-icon" id="back-btn">
			  <i class="la la-arrow-left"></i> <span>Back</span>
		  </a>
	  @else
		  <a  href="{{route('cms.list')}}" style="margin-left:3px;"class="btn btn-primary btn-icon" id="back-btn">
			  <i class="la la-arrow-left"></i> <span>Back</span>
		  </a>
	  @endif
   </div>
